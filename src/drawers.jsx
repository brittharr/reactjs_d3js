...
var HistogramBar = React.createClass({
    render: function() {
        var translate = "translate("+this.props.x+","+this.props.y+")",
            label = this.props.percent < 1 ? this.props.percent.toFixed(2)+'%' : this.props.percent.toFixed(0)+'%';

        if (this.props.width < 10) {
            label = "";
        } else if (this.props.width < 20) {
            label = label.replace("%", "");
        }
...
var Axis = React.createClass({
    componentWillMount: function() {
        ...
        this.axis = d3.svg.axis()
            .scale(this.yScale)
            .orient("left")
            .tickFormat(function(d) { return "$"+this.yScale.tickFormat()(d); }.bind(this));
    ...
    update_d3: function(props) {
        ...
        this.axis
            ...
            .tickValues(props.data.map(function(d){ return d.x; }).concat(props.data[props.data.length-1].x+props.data[props.data.length-1].dx));
    },
